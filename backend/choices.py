...
def add_choice_made(current_user):
  ...
  return jsonify({'message': 'Decisions added to user\'s choice successfully'})
...
def user_adds_friend(current_user):
  ...
  if not user:
    ...
    if friend and friend.selection_option == 1:
      return make_response(403, {'message': 'You are already friends with this user!'})
    elif friend and friend.selection_option == 0:
      return jsonify({'message': 'You have a pending incoming/outgoing friend request with this person'}), 403
    else:
      ...
      try:
        ...
        return jsonify({'message': 'Something went wrong'})
      return jsonify({'message': 'You have successfully sent a friend request to this user!'})
...
def user_accepts_friend(current_user):
  ...
  if not friend_request:
    return jsonify({'message': 'Friend request non-existent or removed'}), 403
    ...
    return jsonify({'message': 'You are already friends with this user'}), 403
  return jsonify({'message': 'Friend request accepted!'}), 200

# Get User's incoming friend requests
@app.route('/incoming_friend_requests', methods=['GET'])
@token_guard
def incoming_friend_requests(current_user):
  incoming_requests = User.query.filter_by(user_id=current_user.user_id).first().friend_to
  incoming_requests_pending = []
  if incoming_requests:
    for req in incoming_requests:
      if req.selection_option == 0:
        friend_data = {}
        friend_data['username'] = req.user_from.username
        friend_data['user_id'] = req.user_from.user_id
        incoming_requests_pending.append(friend_data)
  return jsonify(incoming_requests_pending)

# Get User's outgoing friend requests
@app.route('/outgoing_friend_requests', methods=['GET'])
@token_guard
def outgoing_friend_requests(current_user):
  outgoing_requests = User.query.filter_by(user_id=current_user.user_id).first().friend_from
  outgoing_requests_pending = []
  if outgoing_requests:
    for req in outgoing_requests:
      if req.selection_option == 0:
        friend_data = {}
        friend_data['username'] = req.user_to.username
        friend_data['user_id'] = req.user_to.user_id
        outgoing_requests_pending.append(friend_data)
  return jsonify(outgoing_requests_pending)
